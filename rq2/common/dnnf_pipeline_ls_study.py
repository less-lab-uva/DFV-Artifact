...
def execute_dnnv_properties(dnn_onnx_path, backend_name, properties_path, results_path, std):
...
def main():
    if len(sys.argv) < 7:
        print("Need: Dnn path, Decoder path, model.onnx path, properties path, results path, falsifier name and std")
        return
    ...
    execute_dnnv_properties(dnn_onnx_path, backend_name, properties_path, results_path, std)
